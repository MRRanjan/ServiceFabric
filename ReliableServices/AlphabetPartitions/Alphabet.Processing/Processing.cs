...
namespace Alphabet.Processing
{
    ...
    public class Processing : StatefulService
    {
        ...
        {
            instanceId = $"{Process.GetCurrentProcess().Id}#{context.ReplicaId}";
            ServiceEventSource.Current.Message($"Alphabet.Processing.Processing.Ctor: {instanceId} / {++instanceCount}");
        }

        protected override IEnumerable<ServiceReplicaListener> CreateServiceReplicaListeners()
        {
            ServiceEventSource.Current.Message($"Alphabet.Processing.Processing.CreateServiceReplicaListeners: {instanceId} / {++instanceCount}");
            return new[] { new ServiceReplicaListener(context => this.CreateInternalListener(context))};
        }

        private ICommunicationListener CreateInternalListener(ServiceContext context)
        {
            ServiceEventSource.Current.Message($"Alphabet.Processing.Processing.CreateInternalListener: {instanceId} / {++instanceCount}");
        ...
        {
            ServiceEventSource.Current.Message($"Alphabet.Processing.Processing.ProcessInternalRequest: {instanceId} / {++instanceCount}");
            string output = null;
            ...
            try
            {
                ...
                output = $"<h4>Processing.ProcessInternalRequest:</h4> {instanceId} / {instanceCount}<br>Response " + output;
            }
            catch (Exception ex)
            {
                output = ex.Message;
        ...
        {
            ServiceEventSource.Current.Message($"Alphabet.Processing.Processing.AddUserAsync: {instanceId} / {++instanceCount}");
